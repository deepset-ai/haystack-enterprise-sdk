...
def haystack_pin(config_yaml: str) -> Optional[str]:
    ...
    The inverse of :func:`_build_dependency_block`. The platform picks the validation worker's
    environment from a request header, but rejects a header that disagrees with the YAML's own pin,
    so the header has to be read back off the block rather than sniffed separately.
